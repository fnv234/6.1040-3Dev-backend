# concept: ReportSynthesis[OrgGraph, FormTemplate, AccessCode]
  ...
  **actions**
    ...
    draftSummaryLLM (responseSet: ResponseSet, themes: Set<Text>, teamName?: String, roleDistribution?: Record<String, Number>, totalResponses?: Number): (draft: Text)
      **requires** responseSet exists
      **effects** generate an LLM-assisted narrative summary highlighting themes and metrics with team context for personalized insights
    ...
    generateCompleteReport (formTemplateId: FormTemplateID, responses: Set<Response>, anonymityFlag?: Boolean, kThreshold?: Number, teamName?: String): (report: Report)
      **requires** user is the creator of the form template
      **effects** creates response set, applies anonymity, extracts themes, generates narrative summary with team context, and returns complete report
